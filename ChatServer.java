...
public class ChatServer {
    ...
    private static final String REGISTER_PUBLIC_KEYS_CMD = "REGISTER_PUBLIC_KEYS:";
    private static final String DOUBLE_RATCHET_MESSAGE_CMD = "DOUBLE_RATCHET_MESSAGE:";
    ...
    private static void handleEncryptedMessage(DatagramSocket socket, String msg, String clientIdentifier, InetAddress address, int clientPort) throws Exception {
        ...
        // Logout message handling
         if (decryptedMsg.equals("LOGOUT")) {
            handleLogout(socket, clientIdentifier, address, clientPort);
            return;
        }
        ...
        } else if (decryptedMsg.startsWith(REGISTER_PUBLIC_KEYS_CMD)) {
            handleRegisterPublicKeys(socket, decryptedMsg, clientIdentifier, address, clientPort, currentSessionKey, currentIvSpec);
        } else if (decryptedMsg.startsWith(DOUBLE_RATCHET_MESSAGE_CMD)) {
    ...
    private static void handleLogout(DatagramSocket socket, String clientIdentifier, 
                               InetAddress address, int clientPort) throws Exception {
        String username = authenticatedClients.get(clientIdentifier);
        
        if (username != null) {
            // Remove client from authenticated clients
            authenticatedClients.remove(clientIdentifier);
            
            // Clear session keys
            clientSessionKeys.remove(clientIdentifier);
            clientIvSpecs.remove(clientIdentifier);
            clientDhPublicKeys.remove(clientIdentifier);
            clientNonces.remove(clientIdentifier);
            serverNonces.remove(clientIdentifier);        
            
            log("User logged out: " + username + " from " + clientIdentifier);
            
            // Send Logout confirmation 
            Key sessionKey = clientSessionKeys.get(clientIdentifier);
            IvParameterSpec ivSpec = clientIvSpecs.get(clientIdentifier);
            if (sessionKey != null && ivSpec != null) {
                send(socket, encrypt("LOGOUT_OK", sessionKey, ivSpec), address, clientPort);
            }
        } else {
            log("Logout attempt from non-authenticated client: " + clientIdentifier);
        }
    }
    ...
    private static void handleAuthChallenge(DatagramSocket socket, String decryptedMsg, String clientIdentifier, InetAddress address, int clientPort, Key sessionKey, IvParameterSpec ivSpec) throws Exception {        
        // Client'tan sadece nonce geliyor (timestamp yok)
        String encodedClientNonce = decryptedMsg.substring(DH_AUTH_CHALLENGE_CMD.length());
        BigInteger clientNonce = new BigInteger(Base64.getDecoder().decode(encodedClientNonce));

        log("Received client nonce from " + clientIdentifier);
        clientNonces.put(clientIdentifier, clientNonce);

        // Server challenge to client - server timestamp'i ekleniyor
        ...
        log("Sent authentication challenge with format clientNonce:serverTimestamp:serverNonce to " + clientIdentifier);
    }
    ...
    private static void handleAuthResponse(DatagramSocket socket, String decryptedMsg, String clientIdentifier, InetAddress address, int clientPort) throws Exception {
        // Client'tan server nonce + client timestamp geliyor - Format: serverNonce:clientTimestamp
        String responseData = decryptedMsg.substring(DH_AUTH_RESPONSE_CMD.length());
        String[] parts = responseData.split(":", 2);
        
        if (parts.length != 2) {
            send(socket, SIGNIN_FAIL + "Invalid response format.", address, clientPort);
            log("Authentication FAIL (invalid format) for " + clientIdentifier);
            return;
        }

        BigInteger receivedServerNonce = new BigInteger(Base64.getDecoder().decode(parts[0]));
        String clientTimestampStr = new String(Base64.getDecoder().decode(parts[1]));

        // Server nonce kontrolü
        ...
        // Client timestamp kontrolü
        LocalDateTime clientTimestamp;
        try {
            clientTimestamp = LocalDateTime.parse(clientTimestampStr, TIMESTAMP_FORMATTER);
        } catch (Exception e) {
            log("Invalid client timestamp format from " + clientIdentifier + ": " + clientTimestampStr);
            send(socket, SIGNIN_FAIL + "Invalid timestamp format", address, clientPort);
            return;
        }

        if (Duration.between(clientTimestamp, LocalDateTime.now()).abs().getSeconds() > 60) {
            send(socket, SIGNIN_FAIL + "Client timestamp expired", address, clientPort);
            log("Authentication failed: Client timestamp expired for " + clientIdentifier);
            return;
        }
    ...
    private static void handleSignup(DatagramSocket socket, String decryptedMsg, String clientIdentifier, InetAddress address, int clientPort, Key sessionKey, IvParameterSpec ivSpec) throws Exception {
        ...
        } else if (!isValidPassword(password).equals("true")) {
            send(socket, encrypt(SIGNUP_FAIL + "Password does not meet security requirements " + isValidPassword(password), sessionKey, ivSpec), address, clientPort);
            log("SIGNUP FAIL (weak password): " + username);
        }else {
    ...
    private static void handleRegisterPublicKeys(DatagramSocket socket, String decryptedMsg, String clientIdentifier, InetAddress address, int clientPort, Key sessionKey, IvParameterSpec ivSpec) throws Exception {
        String keyBundle = decryptedMsg.substring(REGISTER_PUBLIC_KEYS_CMD.length());
        String username = authenticatedClients.get(clientIdentifier);
        ...
        for (String clientId : authenticatedClients.keySet()) {
            if (authenticatedClients.get(clientId).equals(recipient)) {
                try {
                    ...
                    String forwardedMsg = DOUBLE_RATCHET_MESSAGE_CMD + sender + ":" + dhPublicKey + ":" + messageNumber + ":" + encryptedMsg;
        ...
        try (PreparedStatement selectStmt = db.prepareStatement("SELECT sender, dh_public_key, message_number, encrypted_message FROM offline_messages WHERE recipient = ? ORDER BY timestamp");
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    String forwardedMsg = DOUBLE_RATCHET_MESSAGE_CMD + sender + ":" + dhPublicKey + ":" + messageNumber + ":" + encryptedMessage;
    ...
    private static String isValidPassword(String password) {
        ...
        if (password == null) {
            ...
            return "Password must be between 12 and 128 characters long";
        ...
        } else if (containsWeakPatterns(password)) {
        ...
        } else if (hasKeyboardPattern(password)) {
            return "Password contains keyboard patterns";
        }
        return "true"; // Password is valid
    }
    
    // Zayıf password pattern kontrolü
    private static boolean containsWeakPatterns(String password) {
        String[] weakPatterns = {
            "password", "123456", "qwerty", "abc123", "admin", 
            "letmein", "welcome", "monkey", "dragon", "master",
            "iloveyou", "sunshine", "princess", "football", "login",
            "passw0rd", "p@ssword", "p@ssw0rd", "secret", "user",
            "test", "guest", "root", "administrator", "demo"
        };
    ...
    private static boolean hasSequentialChars(String password, int minLength) {
        ...
        for (int i = 0; i <= lowerPassword.length() - minLength; i++) {
            ...
            // ASCII değerlerinde ardışık kontrolü
            for (int j = 1; j < minLength; j++) {
                char current = lowerPassword.charAt(i + j);
                char previous = lowerPassword.charAt(i + j - 1);
                
                // Ardışık karakter kontrolü (a->b->c veya 1->2->3)
        ...
        // Ters ardışık kontrol (321, cba, zyx gibi)
        for (int i = 0; i <= lowerPassword.length() - minLength; i++) {
            ...
            for (int j = 1; j < minLength; j++) {
                ...
                // Ters ardışık karakter kontrolü (c->b->a veya 3->2->1)
                if (current != previous - 1) {
    ...
    // Klavye pattern kontrolü (qwerty, asdf, zxcv gibi)
    private static boolean hasKeyboardPattern(String password) {
        // Klavye sıraları (QWERTY layout)
        String[] keyboardRows = {
            "qwertyuiop",     // Üst sıra
            "asdfghjkl",      // Orta sıra
            "zxcvbnm",        // Alt sıra
            "1234567890",     // Sayı sırası
            "!@#$%^&*()",     // Shift + sayı sırası
            "`~-_=+[{]}\\|;:'\",<.>/?"  // Diğer özel karakterler
        };
        
        String lowerPassword = password.toLowerCase();
        
        for (String row : keyboardRows) {
            // 3+ karakter klavye patternlerini kontrol et
            for (int i = 0; i <= row.length() - 3; i++) {
                String pattern = row.substring(i, i + 3);
                
                // Normal pattern kontrolü (qwe, asd, 123)
                if (lowerPassword.contains(pattern)) {
                    return true;
                }
                
                // Ters pattern kontrolü (ewq, dsa, 321)
        ...
        // Dikey klavye patternleri (qaz, wsx, edc)
