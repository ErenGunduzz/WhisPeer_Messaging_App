...
public class ChatServer {
            ...
            if (msg.startsWith(DH_PUBLIC_KEY_CMD)) {
                ...
                if (decryptedMsg.startsWith(DH_AUTH_CHALLENGE_CMD)) {
                    ...
                    try {
                        ...
                        send(socket, encrypt(SIGNIN_FAIL + "Invalid timestamp format", currentSessionKey, currentIvSpec), address, clientPort);
                        continue;
                    }

                    if (Duration.between(clientTimestamp, LocalDateTime.now()).abs().getSeconds() > 60) {
                        send(socket, encrypt(SIGNIN_FAIL + "Timestamp expired", currentSessionKey, currentIvSpec), address, clientPort);
                    ...
                    String encryptedResponse = encrypt(DH_AUTH_CHALLENGE_CMD + responseNonces, currentSessionKey, currentIvSpec);
                    ...
                    if (userExists(username)) {
                        send(socket, encrypt(SIGNUP_FAIL + "Username already exists", currentSessionKey, currentIvSpec), address, clientPort);
                        ...
                        send(socket, encrypt(SIGNUP_OK, currentSessionKey, currentIvSpec), address, clientPort);
                    ...
                    if (result != null) {
                        ...
                        if (storedHash.equals(inputHash)) {
                            ...
                            send(socket, encrypt(SIGNIN_OK, currentSessionKey, currentIvSpec), address, clientPort);
                            log("User signed in: " + username + " from " + clientIdentifier);
                            
                            // Send public key bundles of all other authenticated users
                            sendAllUserKeyBundles(socket, clientIdentifier, username, currentSessionKey, currentIvSpec);
                            ...
                            send(socket, encrypt(SIGNIN_FAIL + "Invalid credentials", currentSessionKey, currentIvSpec), address, clientPort);
                            log("SIGNIN FAIL (wrong password): " + username);
                        }
                    } else {
                        send(socket, encrypt(SIGNIN_FAIL + "User not found", currentSessionKey, currentIvSpec), address, clientPort);
                    ...
                    if (username != null) {
                        ...
                        if (verifySignedPreKeySignature(keyBundle)) {
                            ...
                            send(socket, encrypt("ERROR: Invalid key signature", clientSessionKeys.get(clientIdentifier), clientIvSpecs.get(clientIdentifier)), address, clientPort);
                        }
                    }
                ...
                } else if (decryptedMsg.startsWith("BROADCAST_CMD")) {
                    if (!authenticatedClients.containsKey(clientIdentifier)) {
                        log("Client not authenticated: " + clientIdentifier);
                        continue;
                    }
                    String content = decryptedMsg.substring(BROADCAST_CMD.length());
                    String sender = authenticatedClients.get(clientIdentifier);
                    String outMsg = "<from " + sender + "> " + content;
                    
                    // Send to all authenticated clients
                    for (String targetClientId : authenticatedClients.keySet()) {
                        try {
                            String[] ipPort = targetClientId.split(":");
                            InetAddress targetIP = InetAddress.getByName(ipPort[0]);
                            int targetPort = Integer.parseInt(ipPort[1]);
                            Key targetSessionKey = clientSessionKeys.get(targetClientId);
                            IvParameterSpec targetIvSpec = clientIvSpecs.get(targetClientId);
                            
                            if (targetSessionKey == null || targetIvSpec == null) {
                                log("Session key or IV missing for client: " + targetClientId);
                                continue;
                            }
                            String encryptedOut = encrypt(outMsg, targetSessionKey, targetIvSpec);
                            if (encryptedOut == null) {
                                log("Encryption failed for broadcast message to " + targetClientId);
                                continue;
                            }
                            send(socket, encryptedOut, targetIP, targetPort);
                        } catch (Exception e) {
                            log("Error sending broadcast message to client " + targetClientId + ": " + e.getMessage());
                        }
                    }
                    log("Broadcast sent from " + sender + ": " + content);
    ...
    private static void initDatabase() throws SQLException {
        ...
        try (Statement stmt = db.createStatement()) {
            ...
            // Offline mesajları sakla - Double Ratchet format
    ...
    // Offline mesajları getir ve sil - Double Ratchet format
    ...
    private static void forwardDoubleRatchetMessage(DatagramSocket socket, String sender, String recipient, String dhPublicKey, String messageNumber, String encryptedMsg) {
        ...
        // Recipient offline ise mesajı kaydet
        if (!recipientFound) {
            try {
