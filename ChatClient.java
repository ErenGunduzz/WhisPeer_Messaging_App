...
public class ChatClient {
    ...
    try {
        ...
        String encryptedLogout = encrypt(logoutMessage);
        send(encryptedLogout);
        
        // Wait response from server
        Thread.sleep(100); // Small time for server to process logout
    } catch (Exception e) {
    ...
    private void performSharedKeyTwoWayAuthentication() throws Exception {
        ...
        // Client nonce oluşturuluyor (timestamp YOK)
        clientNonce = new BigInteger(128, new SecureRandom());
        System.out.println("nonce " + clientNonce.toString(16) + " generated from client.");

        // İlk challenge - sadece client nonce gönder (timestamp YOK)
        ...
        // Server'dan gelen değerleri parse et
        BigInteger receivedClientNonce = new BigInteger(Base64.getDecoder().decode(nonceParts[0]));
        String serverTimestampStr = new String(Base64.getDecoder().decode(nonceParts[1]));
        serverNonce = new BigInteger(Base64.getDecoder().decode(nonceParts[2]));
        ...
        // Client nonce doğrulaması
        if (!receivedClientNonce.equals(clientNonce)) {
            throw new IOException("Client nonce mismatch! Potential replay attack.");
        }

        System.out.println("Client nonce and server timestamp verified.\nServer nonce received.");

        // Final response - server nonce + client timestamp
        String clientTimestamp = LocalDateTime.now().format(TIMESTAMP_FORMATTER);
        String authResponseMsg = DH_AUTH_RESPONSE_CMD + 
                Base64.getEncoder().encodeToString(serverNonce.toByteArray()) + ":" +
                Base64.getEncoder().encodeToString(clientTimestamp.getBytes());
    ...
    private void registerPublicKeys() throws Exception {
        ...
        String registerKeys = REGISTER_PUBLIC_KEYS_CMD + publicKeyBundle;
    ...
    private void sendDirectMessage(String recipient, String msg) throws IOException {
        ...
        try {
            ...
            String message = DOUBLE_RATCHET_MESSAGE_CMD + recipient + ":" + dhPublicKey + ":" + state.sendingMessageNumber + ":" + encryptedMsg;
        ...
        while (true) {
            try {
                ...
                if (decrypted != null) {
                    ...
                    } else if (decrypted.startsWith(DOUBLE_RATCHET_MESSAGE_CMD)) {
