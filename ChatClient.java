...
public class ChatClient {
        ...
        while (true) {
            ...
            if (decryptedResponse.startsWith(SIGNUP_OK) || decryptedResponse.startsWith(SIGNIN_OK)) {
                ...
                try {
                    ...
                    System.err.println("Error registering public keys: " + e.getMessage());
                }
                break;
            } else {
    ...
    private byte[] deriveMessageKey(byte[] chainKey, int messageNumber) {
        try {
            ...
            mac.update(("MESSAGE_KEY_" + messageNumber).getBytes());
    ...
    private void sendBroadcast(String msg) throws IOException {
        String message = BROADCAST_CMD + msg;
        String encrypted = encrypt(message);
        send(encrypted);
    }
            ...
            try {
                ...
                if (decrypted != null) {
                    if (decrypted.startsWith("USER_KEY_BUNDLE:")) {
                        ...
                        // Regular message display (broadcast etc.)
